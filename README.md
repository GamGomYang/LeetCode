...
아래 예시는 **Grind 75 (169문제)** 전체 목록을,  
사용자님이 원하시는 **표 형식(번호, 제목, 언어, 풀이 링크, 난이도)** 으로 구성한 README 예시입니다.

- **풀이 링크**는 비워 두었으니, 실제 풀이를 업로드하신 뒤에 해당 칸에 링크를 달아주시면 됩니다.
- **언어 배지**(Python, Java, Kotlin 등)는 일단 전부 Java로 설정했습니다. (필요 시 수정 가능)
...
1. 원하는 **카테고리**를 찾은 뒤, **문제 번호 / 제목**을 확인합니다.  
2. 풀이를 추가하셨다면 **풀이 링크** 칸을 수정하여 연결해 주세요.  
3. 난이도별, 혹은 자신이 선호하는 **언어 배지**(Java, Python, Kotlin 등)로 자유롭게 변경 가능합니다.

## 기여 가이드
- **새로운 문제 풀이**: 문제 번호별 폴더/파일 생성 후, 이 README 표의 해당 문제 행에 **풀이 링크**를 추가하세요.
- **오탈자**나 **내용 보완** 등은 PR(풀 리퀘스트)을 통해 자유롭게 기여해주세요.

---

**모두 즐코딩 하시길 바랍니다!**  
궁금한 점이나 피드백 있으면 언제든 말씀해 주세요. 
